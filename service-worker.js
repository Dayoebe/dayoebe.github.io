const APP_CACHE = 'wireless-terminal-app-v16';
const STATIC_CACHE = 'wireless-terminal-static-v16';
const THIRD_PARTY_CACHE = 'wireless-terminal-third-party-v16';
...
async function handleNavigationRequest(request) {
  try {
    ...
    if (networkResponse && networkResponse.ok) {
      const responseToReturn = await injectPortfolioEnhancements(networkResponse.clone());
      const appCache = await caches.open(APP_CACHE);
    ...
    if (cachedResponse) {
      return injectPortfolioEnhancements(cachedResponse);
    }
...
async function injectPortfolioEnhancements(response) {
  ...
  if (html.includes(PORTFOLIO_ENHANCEMENT_SCRIPT) || !html.includes('</body>')) {
  ...
  const enhancedHtml = html.replace(
    '</body>',
    `<script defer src="${PORTFOLIO_ENHANCEMENT_SCRIPT}"></script>\n</body>`
  );
